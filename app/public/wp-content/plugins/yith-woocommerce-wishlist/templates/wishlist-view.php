...
<table
	...
	<script>
		...
		window.dataLayer.push({
			...
			items: [
				...
				$cats = array_slice( array_pad( $cats, 4, '' ), 0, 4 );
				$dl_items[] = [
					'item_id'                 => $prod->get_id(),
					'item_name'               => $prod->get_name(),
					'sku'                     => $prod->get_sku(),
					'price'                   => $prod->get_price(),
					'stocklevel'              => $prod->get_stock_quantity() ?: 0,
					'stockstatus'             => $prod->is_in_stock() ? 'instock' : 'outofstock',
					...
					'id'                      => $prod->get_id(),
					'item_brand'              => $prod->get_attribute( 'brand' ),
				...
				echo wp_json_encode( $dl_items, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE );
				?>
			]
			}
		});
	...
	if ( $wishlist && $wishlist->has_items() ) :
		$product_listitems = $wishlist->get_items();
