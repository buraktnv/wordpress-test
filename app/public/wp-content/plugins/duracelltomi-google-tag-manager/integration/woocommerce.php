...
function gtm4wp_woocommerce_process_order_items( $order ) {
    ...
    if ( $order_items ) {
        foreach ( $order_items as $order_item ) {
            // allow 3rd-party filters to exclude items
            ...
            if ( $eec_product_array ) {
                ...
                $eec_product_array['currency'] = $currency;

                // Unit price (no discount) & sale price
                $eec_product_array['unit_price']      = isset( $eec_product_array['unit_price'] )
                    ? $eec_product_array['unit_price']
                    : $product_price;
                $eec_product_array['unit_sale_price'] = $product_price;

                // Product URL
                $eec_product_array['url'] = $product
                    ? get_permalink( $product->get_id() )
                    : '';

                // Stock level (if managed)
                if ( $product && $product->managing_stock() ) {
                    $eec_product_array['stock'] = $product->get_stock_quantity();
                }

                // Product image
                if ( $product ) {
                    $image_id = $product->get_image_id();
                    $eec_product_array['product_image_url'] = $image_id
                        ? wp_get_attachment_url( $image_id )
                        : '';
                }

                // Color & Size (for variations)
                if ( $product && $product->is_type( 'variation' ) ) {
                    $vars = $product->get_variation_attributes();
                    $eec_product_array['color'] = ! empty( $vars['attribute_pa_color'] )
                        ? $vars['attribute_pa_color']
                        : '';
                    $eec_product_array['size']  = ! empty( $vars['attribute_pa_size'] )
                        ? $vars['attribute_pa_size']
                        : '';
                }

                // Taxonomy (breadcrumb) fallback
                if ( empty( $eec_product_array['taxonomy'] ) ) {
                    $eec_product_array['taxonomy'] = array();
                    ...
                    if ( $terms && ! is_wp_error( $terms ) ) {
                        ...
                        foreach ( $terms as $term ) {
                            ...
                            foreach ( array_reverse( $ancestors ) as $aid ) {
                                $anc_term = get_term( $aid, 'product_cat' );
                                if ( ! is_wp_error( $anc_term ) ) {
                                    $path[] = $anc_term->name;
                                }
                            }
                            $path[] = $term->name;
                        ...
                        $eec_product_array['taxonomy'] = $best_path;
                    }
                }

                $eec_product_array['quantity'] = $quantity;
                $eec_product_array['subtotal'] = round( $product_price * $quantity, 2 );
...
function gtm4wp_woocommerce_datalayer_filter_items( $data_layer ) {
	...
	if (
		...
		foreach ( $current_cart->get_cart() as $cart_item_id => $cart_item_data ) {
			...
			// 1) Grab the image URL
			$image_url = '';
			if ( $image_id = $product->get_image_id() ) {
				$image_url = wp_get_attachment_url( $image_id );
			}
	
			// 2) Pull variation attributes from the cart data (if any)
			$size_attr  = '';
			$color_attr = '';
			if ( ! empty( $cart_item_data['variation'] ) ) {
				$vars = $cart_item_data['variation'];
				if ( isset( $vars['attribute_pa_size'] ) ) {
					$size_attr = $vars['attribute_pa_size'];
				}
				if ( isset( $vars['attribute_pa_color'] ) ) {
					$color_attr = $vars['attribute_pa_color'];
				}
			}
			...
			// 4) Build the GA4 product array, passing in our extras
			$eec_product_array = gtm4wp_woocommerce_process_product(
				$product,
				array(
			...
			// We don’t need internal_id in the frontend push
			unset( $eec_product_array['internal_id'] );
		...
		if ( $gtm4wp_options[ GTM4WP_OPTION_INTEGRATE_WCTRACKECOMMERCE ] ) {
			...
			switch ( $data_layer['productType'] ) {
				...
				default:
					...
					$eec_product_array['image'] = ! empty( $image_src[0] ) ? $image_src[0] : '';
