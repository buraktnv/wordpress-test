...
function gtm4wp_add_basic_datalayer_data( $data_layer ) {
	...
	if (yith_wcwl_is_wishlist_page())	{
		$data_layer['pagePostType'] = 'wishlist';
	}

	if (is_product_category()) {
		$data_layer['pagePostType'] = 'category';
	}

	if (is_cart()) {
		$data_layer['pagePostType'] = 'cart';
	}

	if (is_checkout()) {
		$data_layer['pagePostType'] = 'checkout';
	}

	if ( is_front_page() && $gtm4wp_options[ GTM4WP_OPTION_INCLUDE_POSTTYPE ] ) {
		$data_layer['pagePostType'] = 'home';
	}
	...
	if ( is_404() ) {
		$data_layer['pagePostType'] = 'not-found';
	}
